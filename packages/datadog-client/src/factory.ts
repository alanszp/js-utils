import { BufferedMetricsLogger, BufferedMetricsLoggerOptions } from "datadog-metrics";
...
function createDatadogClient(config: DatadogClientConfig): IDatadogClient {
    const opts: BufferedMetricsLoggerOptions = {
        ...
        flushIntervalSeconds: config.flushIntervalSeconds || DEFAULT_FLUSH_INTERVAL_SECONDS,
        defaultTags: [
            `env:${config.env}`,
            ...(config.defaultTags || []),
        ],
